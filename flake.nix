    ...
    in
    {
      packages = forEachSystem (pkgs: {
        x56d = pkgs.stdenv.mkDerivation {
          ...
          buildPhase = ''
            gcc -o x56d -Wall -std=c99 \
              src/main.c src/usb.c \
              $(${pkgs.pkg-config}/bin/pkg-config --cflags --libs libusb-1.0)
          '';
          installPhase = ''
            install -Dm755 x56d $out/bin/x56d
#            install -Dm644 udev/99-x56.rules $out/lib/udev/rules.d/99-x56.rules
          '';
        };
      ...
      devShells = forEachSystem (pkgs: {
        default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            clear
            export PKG_CONFIG_PATH="${pkgs.libusb1.dev}/lib/pkg-config:$PKG_CONFIG_PATH"
            echo "x56d dev shell"
